...
#SBATCH --time 2:00:00
#SBATCH --array=1-11
#SBATCH --job-name mergeRepMAnorm
#SBATCH --chdir /scratch/ldelisle/AmandioEtAl2021/ChIPM/

gitHubDirectory=$1

# This script perform average between replicates for MAnorm
...
n=$(echo $samples | wc -w)
sample="${sampleName}_MAnorm_neq$n"
...
allBDG=""
for s in $samples; do
  bedgraph=$(ls ${path}/${s}_macs_SR200_withPseudoCount*.bedGraph.gz)
  if [ -z $bedgraph ]; then
    echo "No file match ${s}_macs_SR200_withPseudoCount*.bedGraph.gz"
    exit 1 
  fi
  if [ $(echo $bedgraph | wc -w) -ne 1 ]; then
    echo "Found more than one file: $bedgraph"
    bedgraph=$(ls ${path}/${s}_macs_SR200_withPseudoCount*.bedGraph.gz | head -n 1)
    echo "Will use: $bedgraph"
  fi
  zcat $bedgraph > normalized_${s}.bdg
